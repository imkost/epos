...
function createContinuous (sourceArray, fn) {
  ...
  const onAdd = (index, item) => {
    continuous.splice$(index, 0, createSource(fn(item)))
  }

  const onRemove = (index) => {
    continuous.splice$(index, 1)
  }

  sourceArray[__itemAdd].add(onAdd)
  sourceArray[__itemRemove].add(onRemove)

  if (curAutorun) {
    curAutorun.children.push({
      stop () {
        sourceArray[__itemAdd].delete(onAdd)
        sourceArray[__itemRemove].delete(onRemove)
      }
    })
  }
...
function renderContinuous (array, fn) {
  ...
  array[__itemAdd].add(onAdd)
  array[__itemRemove].add(onRemove)

  if (curAutorun) {
    curAutorun.children.push({
      stop () {
        array[__itemAdd].delete(onAdd)
        array[__itemRemove].delete(onRemove)
      }
    })
  }
